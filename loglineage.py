...
def batch_write_records(records: list[Record], table: str, spark):
    ...
    df.writeTo(f"AwsDataCatalog`{DB_NAME}`.`{table}`") \
        .using("iceberg") \
...
def create_iceberg_spark_session():
    ...
    all_jars = ",".join(str(jar) for jar in jars_dir.glob("*.jar"))
    ...
    spark = SparkSession.builder \
        ...
        .config("spark.sql.catalog.AwsDataCatalog.type", "iceberg") \
        .config("spark.sql.catalog.AwsDataCatalog.warehouse", "s3a://apache-iceberg-datalineage-533267385393-tst/" ) \
